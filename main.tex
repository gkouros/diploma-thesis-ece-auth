...
Autonomy is the most important property of any robotic system, regarding its operation that aims at the completion of a goal or task, without any human interference. Autonomous Mobile Robots, in particular, must be able to navigate autonomously in their environments, in order to be able to perform given tasks, which, as a result, requires the addressing of a wide range of problems. Such problems are usually addressed using defferent approaches, depending on the capabilities and limitations of the respective robotic vehicle.

\bigskip
As part of this thesis, a robotic vehicle, was developed, that adheres to a non ideal Four-Wheel-Steering kinematic model, that presents a set of kinematic constraints that make the utilization of popular motion and navigation algorithms, impossible. The aforementioned robotic vehicle, was equipped with sensors that allow it to perceive its environment, as well as actuators that allow it to navigate through it, while, at the same time, adhering to the kinematic model that was developed for it and which takes into consideration a number of assumptions and approximations.
